...
const createTweetElement = function(data) {
  ...
  $header.append(`<img class="user-avatar" src=${data.user.avatars.small}>`)
         .append(`<h2 class="user-full-name">${data.user.name}</h2>`)
         .append(`<span class="username">${data.user.handle}</span>`);
  ...
  $footer.append(data['created_at']);

  return $article;
}

let renderTweets = function(tweetsArray) {
  tweetsArray.forEach( (tweet) => {
    let formattedTweet = createTweetElement(tweet);
